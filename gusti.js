...
const vcard = 'BEGIN:VCARD\n'  // Gak Usah Di Ganti
            ...
            + 'FN:KAZEO BOTZ\n' //ganti nama bot lu
            + 'ORG: KAZEO GANZ;\n' //Ganti Jadi Nama Lu
            + 'TEL;type=CELL;type=VOICE;waid=6285706666337:+6285706666337\n'  //ganti nomor lu! 
            + 'END:VCARD'
prefix = '/' // Ganti Aja Terserah Lu 
blocked = []   //Jangan Ganti
limitawal = 9999999 //Bebas Ganti 
memberlimit = 1 // serah
faketoko = "KAZEO BOTZ" //ganti nama bot
thumb = 'KAZEO BOTZ' //terserah lu
cr = '*KAZEO BOTZ*' //ganti
/******** OWNER NUMBER**********/
const ownerNumber = ["6285839352141@s.whatsapp.net"]   // Ganti Nomor Lu
const pacarNumber = ["62858393521413@s.whatsapp.net"]  // Ganti Jadi Nomor Bot Lu
/************************************/

//========ATUR SEMUA D SNI===========//
namabot = 'KAZEO BOTZ' //Nama Bot lu
github = 'Gusti-ID' //username github
namaowner = 'KazeoGanz' //nama owner
nobot = '62858393521413' //nobot
nomerowner = '6285839352141'
namabot = 'KAZEO BOTZ' //namabot
igbot = '@gamestore.kazeo' //username ig
restlu = 'GAADA' //klo punya isi aj
gruplu = 'https://chat.whatsapp.com/CFBVIGxwgWVE1KgO8kaAQ5' //link gc lu

// Settings Apikey
ojankey = 'OjanKeY'
LolKey = 'e2c14ab01e67ef9820eced22' //register di lolhum
Vhtear = 'NOT-PREMIUM' // Modal :v
mikukey = 'mikubot'
apivinz = 'ApiGustiTzy' //register aja di web https://zeks.xyz
jann = 'ojan'
...
client.browserDescription = [ 'GustiGanz', 'Browser', '999' ]
   client.on('qr', qr => {
   qrcode.generate(qr, { small: true })
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('YT GUSTI GANZツ','red'),color('X','lime'),color('LolHuman','red'))
})

client.on('credentials-updated', () => {
	const authInfo = client.base64EncodedAuthInfo()
   console.log(`credentials updated!`)
   fs.writeFileSync('./gustitzy.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./gustitzy.json') && client.loadAuthInfo('./gustitzy.json')
client.connect();
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! 😠")
                        ...
                        .then(() => {
                            client.sendMessage(from, `*「 ANTI BADWORD 」*\n${namabot} akan kick kamu karena berkata kasar!`, text ,{quoted: mek})
		...
		setTimeout( () => {
			...
			switch(command) {
				...
				case 'gets':  
					...
					client.sendMessage(from, result, sticker, {quoted :ftoko})
					break
				case 'stickerlist':
				case 'liststicker':  
				...
				case 'getvn':  
					...
					client.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: ftoko, ptt: true })
					break
				case 'listvn':
				case 'vnlist':  
  ...
  break	
		...
		case 'transfer':
              ...
              case 'soundplaydate':
				...
				client.sendMessage(from, XYTODmy, MessageType.audio, { quoted: ftoko, ptt: true })
				await limitAdd(sender)
				break
                case 'admin':
				if (!isOwner) return reply(ind.ownerb())
				...
				client.sendMessage(from, teks.trim(), extendedText, {quoted: ftoko, contextInfo: {"mentionedJid": adm}})
				break
                case 'premium':
				if (!isOwner) return reply(ind.ownerb())
				premm = body.slice(9)
		...
		case 'unpremium':
				...
				client.sendMessage(from, teks.trim(), extendedText, {quoted: ftoko, contextInfo: {"mentionedJid": prem}})
				break
                case 'ban':
				bnnd = body.slice(4)
		...
		case 'unban':
				...
				client.sendMessage(from, teks.trim(), extendedText, {quoted: ftoko, contextInfo: {"mentionedJid": ban}})
				break
		...
		case 'cuttly':
				...
				case 'pornhub2':
					...
					var porn = gh.split("/")[0];
					var hub = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}pornhub Gusti|Hub`)
					reply('Wait Tod')
					alan = await getBuffer(`https://vinz.zeks.xyz/api/pornhub?text1=${porn}&text2=${hub}`)
					client.sendMessage(from, alan, image, {quoted: ftoko})
					await limitAdd(sender)
				break
				case 'wasted':
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
  ...
  ger = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo: ftoko
...
if (isBanned) return reply('```Lu kebanned kontol```')
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
				...
				var lion = gh.split("/")[0]
				var logo = gh.split("/")[1]
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}lionlogo Gusti/Senpai`)
				reply('[❕] Loading') 
				buffer = await getBuffer(`https://lolhuman.herokuapp.com/api/textprome2/lionlogo?apikey=${LolKey}&text1=${lion}&text2=${logo}`) 
				client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Follow IG BOT : instagram.com/gustinx77*', quoted: ftoko})
				await limitAdd(sender) 
				break
				case 'pornhub':
				var gh = body.slice(9) 
				var porn = gh.split("/")[0]
				var hub = gh.split("/")[1]
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub Gusti/Hub`)
				reply('[❕] Loading') 
				...
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub Gusti/Develor/Bot`)
				reply('[❕] Loading') 
...
if (isBanned) return reply('```Lu kebanned kontol```')
         ...
         break
				...
				case 'googletext':
					...
					if (args.length < 1) return reply(`Contoh : ${prefix}googletext gusti/gans/abiezz`)
		...
		case 'nulis':
                ...
                client.sendMessage(from, ojan, image, {quoted: ftoko})
                await limitAdd(sender)
                break
		case 'nulis2':
				...
				client.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: ftoko})
				await limitAdd(sender)
				break
		case 'galaxtext':
		...
		case 'bplogo':
				...
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: ftoko})
				await limitAdd(sender)
				break
		case 'tahta':
         if (isBanned) return reply('Maaf Kamu Sudah Terbanned astagfirullah') 
         if (args.length == 0) return reply(`Usage: ${prefix} + command} text\nExample: ${prefix + command} Gusti`) 
         teks = args.join(" ") 
         reply('[❕] Loading') 
		...
		case 'darkneon':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan lupa follow ig instagram.com/gustinx77*', quoted: freply})
         break
        case 'wanted':
	if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
	...
	var imgbb = require('imgbb-uploader')
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ted = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo: ftoko
	  reply('Wait Tod')
	  owgi = await client.downloadAndSaveMediaMessage(ted)
	  tels = body.slice(7)
	  anu = await imgbb("08579d070df9a07cb1c2ee565aece767", owgi)
	  hehe = await getBuffer(`https://videfikri.com/api/textmaker/gtavposter/?urlgbr=${anu.display_url}`)
	 client.sendMessage(from, hehe, image, {quoted:ftoko})
	} else {
	  reply('Jangan tambah kan apapun pada command')
	}
	break
	...
	break
		case 'hartatata3':
				...
				client.sendMessage(from, bh, image, {caption: 'Nih kak udah jadi..', quoted: ftoko})
				await limitAdd(sender)
				break
		case 'text2gif':
		...
		case 'attp':
				...
				client.sendMessage(from, attp2, sticker, {quoted: ftoko})
				break
		case 'ttp':
		...
		case 'shota':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				    try {
						...
						client.sendMessage(from, buffer, image, {quoted: ftoko, caption: 'Ingat! Cintai husbumu'})
					} catch (e) {
				...
				client.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: ftoko})
				fs.unlinkSync(ran)
				})
				break
...
break
         ...
         client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Follow IG BOT : instagram.com/gustin77*', quoted: ftoko})
         break
         ...
         case 'ffstalk':
				...
				id = args.join(' ')
				ngntl = await fetchJson(`https://api.lolhuman.xyz/api/freefire/${id}?apikey=${memek}`)
				hsil = `Nickname : ${ngntl.result}`
				dark.sendMessage(from, hsil, text, {quoted: fkontak})
				await limitAdd(sender)
				break
         case 'lollogo':
         if (isBanned) return reply('Maaf Kamu Sudah Terbanned astagfirullah') 
         ...
         break 
				...
				case 'spamsms':
				if (!isOwner) return reply(ind.ownerb())
				    if (!isUser) return reply(mek.noregis)
				    if (isBanned) return reply(mek.baned)
				    if (isLimit(sender)) return reply(mek.limitend)
		...
		case 'randomkpop':
				...
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: ftoko, caption: 'Nih kpopnya kak...'})
				await limitAdd(sender)
				break
		case 'aesthetic':
		...
		case 'logoepep': 
				...
				client.sendMessage(from, buffer, image, {quoted: ftoko, caption: 'Nih hasilnya kak...'})
				await limitAdd(sender)
				break
		case 'logoepep2': 
				gatauda = body.slice(10)
		...
		case 'pornlogo': 
				...
				client.sendMessage(from, buffer, image, {quoted: ftoko, caption: infomp3})
				client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${data.result.title}.mp3`, quoted: ftoko})
				await limitAdd(sender)
				break
		case 'play':   
		...
		/*case 'play2':   
				...
				client.sendMessage(from, laguu , audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: ftoko})
				await limitAdd(sender)
				break*/
		case 'limit':
		...
		case 'vinta':
				...
				client.sendMessage(from, vintage, image, {caption: 'Nih kak ${vin}', quoted: ftoko})
				await limitAdd(sender)
				break
                case '1cak':
				    try {
                 ...
                 if (!isRegistered) return reply( ind.noregis())
						...
						client.sendMessage(from, buffer, image, {quoted: ftoko, caption: 'Nih Kak udah Jadi'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(ind.wrongf())
					}
  ...
  break
		case 'infocuaca':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, hasil, text, {quoted: ftoko})
					await limitAdd(sender)
					break
		case 'kurs': 
                 if (!isRegistered) return reply( ind.noregis())
		...
		case 'github': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, buffer7, image, {quoted: ftoko, caption: capt})
					await limitAdd(sender)
					break
                case 'slap':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, pod, image, { quoted: ftoko, caption: '*Toxic*\n\n'+ ple })
					await limitAdd(sender)
					break
		case 'tampar':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					exec(`wget ${buffer.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(rano)
						client.sendMessage(from, buffer, sticker, {quoted: ftok})
						fs.unlinkSync(rano)
					})
					await limitAdd(sender)
					break
		...
		case 'brainly':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					client.sendMessage(from, teks, text, {quoted: ftoko, detectLinks: false})
					console.log(res)
					})
					await limitAdd(sender)
					break 
		...
		case 'pinterest':
           ...
           case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}glitchtext Gusti/Gans`)
					reply('Wait Tod')
...
quoted: mek, caption: `Gimana? *Jangan Lupa Follow IG Bot : instagram.com/gustinx77`
  })
  } catch {
    reply(mess.ferr)
  }
			...
			break
               ...
               case 'asupan':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned astagfirullah!')
                    ...
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan Lupa Follow IG BOT : instagram.com/gustinx77*', quoted: mek})
         break
...
if (isBanned) return reply('```Lu kebanned kontol```')
               ...
               case 'ytmp4':
                    ...
                    client.sendMessage(from, buffer, image, { quoted: mek, caption: txt })
                    get_audio = await getBuffer(get_result.link[0].link)
                    client.sendMessage(from, get_audio, video, { mimetype: 'audio/mp3', filename: `${get_result.title}.mp3`, quoted: ftoko })
                    break
...
if (isBanned) return reply('```Lu kebanned kontol```')
		...
		case 'help': 
		case 'menu':
          const _0x348b=['\x0a\x20\x20----------------------------------\x0a\x0a\x0a*','listadmin\x0a►\x20','totalCases','\x0a►\x20Browser\x20:\x20','ytplay\x0a►\x20','limit\x0a►\x20','grup\x20[buka/tutup)\x0a\x0a\x0a\x20\x20*◪\x20OWNER\x20MENU*\x0a►\x20','\x0a►\x20Version\x20:\x20','pornhub\x0a►\x20','78423wTFATb','quotesislami\x0a►\x20','ytmp3\x20\x0a►\x20','13933dHAImA','premiumlist\x0a►\x20','toimg\x0a►\x20','bagaimanakah\x0a►\x20','mining\x0a►\x20','coffecup\x0a►\x20','antilinkyt\x20[1/0]\x0a►\x20','wakillist\x0a►\x20','carbon\x0a►\x20','tts\x0a►\x20','narutobanner\x0a►\x20','watak\x0a►\x20','crossgun\x0a►\x20','happymod\x0a►\x20','info*\x0a*','nulis\x0a►\x20','videolist\x0a►\x20','tupai\x0a►\x20','searchfilm\x0a►\x20','bakar\x0a►\x20','\x0a\x20\x0a[\x20Thanks\x20To\x20]\x0a•\x20Ojan\x0a•\x20LolHuman\x0a•\x20Hans\x0a•\x20Dappa\x0a•\x20GustiGanz\x0a•\x20','setprefix\x0a►\x20','apkpure\x0a►\x20','katacinta\x0a►\x20','aesthetic\x0a►\x20','tribunews\x0a►\x20','pantaimalam\x0a►\x20','lirik\x0a►\x20','kelinci\x0a►\x20','halah\x0a►\x20','coffeecup\x0a►\x20','251029wpSyHY','lib/gusti.jpg','ffserti2\x20*TEKS*\x0a►\x20','\x0a\x20\x20►\x20Duit\x20:\x20*Rp.','spamsms\x20gunakan\x20nomor\x20(08xxxx)\x0a\x0a\x20\x0a\x20*◪\x20STORAGE*\x0a►\x20','bugtroli\x0a►\x20','animecry\x0a\x0a\x0a\x20\x20*◪\x20RANDOM\x20IMAGE*\x0a►\x20','underwater\x0a►\x20','buggc\x0a\x0a\x0a\x20\x20*◪\x20INFORMATION\x20MENU*\x0a►\x20','costumwp\x0a►\x20','asupansantuy\x0a\x20\x0a\x20\x20*◪\x20DOWNLOAD*\x0a►\x20','174238614569481','fml\x0a►\x20','jadwalsholat\x0a►\x20','nickff\x0a\x0a\x0a\x20\x20*◪\x20MAKER\x20MENU*\x0a►\x20','film2\x0a►\x20','add\x20[62]\x0a►\x20','gantengcek\x0a►\x20','memeindo\x0a►\x20','ssweb\x0a►\x20','doraemon\x0a►\x20','waifu\x0a►\x20','attp\x0a►\x20','wanted\x0a►\x20','tiktokstalk\x20\x0a►\x20','foxnews\x0a►\x20','cantikcek\x0a►\x20','\x0a\x0a📄NOTE\x20:\x20\x0aSpam\x20call,Spam\x20command\x20=\x20*BLOCK\x20+\x20BANNED*','lpr*\x0a*','\x0a►\x20Speed\x20:\x20','meme\x0a►\x20','animeimg\x0a►\x20','glowteks\x0a►\x20','AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA==','ocr\x0a►\x20','liputan6\x0a►\x20','haram\x0a►\x20','hidetag\x0a►\x20','cogan\x0a►\x20','bc\x0a►\x20','buypremium*\x0a*','sendMessage','gaycek\x0a►\x20','getsticker\x0a►\x20','ig\x0a►\x20','resepmasakan\x0a►\x20','candlemug\x0a►\x20','lovemsg\x0a►\x20','pokemon\x0a►\x20','clearall\x0a►\x20','kucing\x0a►\x20','bass\x0a►\x20','rahmat\x0a►\x20','level\x0a►\x20','nulis2\x0a►\x20','amongus\x0a►\x20','tinyurl\x0a►\x20','wame\x0a►\x20','addImage\x0a►\x20','19EVOZUa','joox\x0a►\x20','\x0a\x20\x20►\x20Creator\x20:\x20GustiTzy\x0a\x20\x20►\x20Recode\x20:\x20','readFileSync','lesbicek\x0a►\x20','setreply\x0a►\x20','asupan\x0a►\x20','addsticker\x0a►\x20','demote\x20[@tag]\x0a►\x20','infofilm\x0a►\x20','neko\x0a\x20\x0a\x20\x0a\x20\x20*◪\x20QUOTES\x20MENU*\x0a►\x20','spamchat\x20gunakan\x20nomor\x20(62xxxx)\x0a►\x20','alay\x0a►\x20','quotesmotivasi\x0a►\x20','nsfw\x20[1/0]\x0a►\x20','infogempa\x0a►\x20','163769KqEmJR','kalkulator\x0a►\x20','getimage\x0a►\x20','getaudio\x0a►\x20','imagelist\x0a►\x20','rate\x0a►\x20','buypremiumlimit\x0a►\x20','promote\x20[@tag]\x0a►\x20','⊳\x20LORD\x20','event\x20[1/0]\x0a►\x20','\x0a►\x20Handphone\x20:\x20','leave\x0a►\x20','freefireimg\x0a►\x20','glitchtext\x0a►\x20','transfer\x0a►\x20','snk*\x0a*','covidindo\x0a►\x20','browserDescription','leveling\x20[1/0]\x0a►\x20','\x0a\x20►\x20Terakhir\x20Di\x20Update:\x20','\x0a\x20\x20----------------------------------\x0a◪\x20COVID\x20DUNIA\x0a\x20►\x20Kasus:\x20\x20','linkgc\x0a►\x20','\x0a\x20►\x20Meninggal:\x20','sound\x0a►\x20','\x0a\x20\x20►\x20Rest\x20Api\x20:\x20','clone\x0a►\x20','1jxhYzD','\x0a\x20►\x20Sembuh:\x20\x20','suit\x0a\x0a\x20\x0a\x20\x20\x20*◪\x20APK\x20MENU*\x0a►\x20','name','gpl\x0a►\x20','nguli\x0a►\x20','animehug\x0a►\x20','woodblock\x0a►\x20','blocklist\x0a►\x20','\x0a►\x20Server\x20:\x20','◪\x20INFO\x20BOT\x0a\x20\x20►\x20Nama\x20:\x20','ping*\x0a*','moddroid\x0a\x0a\x20\x0a\x20\x20*◪\x20MEDIA\x20MENU*\x0a►\x20','addbadword\x0a►\x20','37324jfxncB','welcome\x20[1/0]\x0a►\x20','addaudio\x0a►\x20','hurufterbalik\x0a►\x20','banlist\x0a►\x20','setpp\x0a►\x20','6289523258649-1604595598@g.us','\x0a\x20\x20►\x20Xp\x20:\x20','lastUpdate','phone','hilih\x0a►\x20','\x20」\x0a\x20\x20►\x20Nama\x20:\x20','split','neontext\x0a►\x20','googletext\x0a►\x20','\x20MENGINTAI\x20⊲','\x20\x0a\x20\x20►\x20Nomer\x20:\x20wa.me/','setname\x0a►\x20','result','ttp\x0a►\x20','maling\x0a►\x20','igstalk\x0a►\x20','kickall\x0a►\x20','142376CEMXCM','pow','joox\x20\x0a►\x20','holoh\x0a►\x20','cecan\x0a►\x20','glowteks\x0a\x0a\x0a*◪\x20SERTIFIKAT\x20MENU*\x0a►\x20','getvideo\x0a►\x20','pubglogo\x0a►\x20','cium\x0a►\x20','hpotter\x0a►\x20','tomp3\x0a►\x20','apakah\x0a►\x20','recovered','shota\x0a►\x20','simih\x20[1/0]\x20\x0a►\x20','antilink\x20[1/0]\x0a►\x20','ww2\x0a►\x20','hargaprem*\x0a\x0a\x0a\x20\x20*◪\x20GROUP\x20MENU*\x0a►\x20','quoteskehidupan\x0a\x0a\x0a\x20*◪\x20LIMIT\x20&\x20UANG*\x0a►\x20','CATALOG','play\x0a\x0a\x20\x0a\x20\x20*◪\x20ANIME\x20MENU*\x0a►\x20','*\x0a\x20\x20►\x20Level\x20:\x20','user','sticker\x0a►\x20','ranime\x0a►\x20','loli\x0a►\x20','bisakah\x0a►\x20','jokerlogo\x0a►\x20','slowmo\x0a►\x20','facebookpage\x0a►\x20','tolol\x20*TEKS*\x0a►\x20','Maaf\x20kamu\x20sudah\x20terbenned!','antilinkig\x20[1/0]\x0a►\x20','leaderboard\x0a\x0a\x0a\x20*◪\x20SOUND\x20MENU*\x0a►\x20','grouplist\x0a►\x20','nobadword\x20[1/0]\x0a►\x20','.-*\x0a\x20\x20►\x20Role\x20:\x20*','quotesnasehat\x0a►\x20','bitly\x0a►\x20','\x0a\x20\x20►\x20Instagram\x20:\x20instagram.com/','infocuaca\x0a►\x20','antilinktele\x20[1/0]\x0a►\x20','https://mikubotz.herokuapp.com/api/covidworld?apikey=mikubot','setppbot\x0a\x0a\x0a*◪\x20BUG\x20MENU*\x0a►\x20','brainly\x20\x0a►\x20','animegirl\x0a►\x20','\x0a\x20►\x20Jam:\x20','tagall\x0a►\x20','husbu\x0a►\x20','\x0a\x20\x0a\x20\x0a────────────\x20\x20\x0a╔╗╔╗╔══╗╔══╗\x20\x20\x0a║╚╝║║╔╗║╚║║╝\x20\x20\x0a║╔╗║║╠╣║╔║║╗\x20\x20\x0a╚╝╚╝╚╝╚╝╚══╝\x20\x20\x0a────────────\x20\x20\x0a\x0a◪\x20YOUR\x20INFO\x0a\x20\x20►\x20Prefix\x20:\x20「\x20','sound2\x0a\x0a\x20\x0a\x20*◪\x20TOOLS*\x0a►\x20','ffserti\x20*TEKS*\x0a►\x20','animeboy\x0a►\x20','11017sclvEH','0@s.whatsapp.net','lollogo\x0a►\x20','21SbeKBg','deaths','calendermaker\x0a►\x20','pantun\x0a►\x20','sangecek\x0a►\x20'];const _0x1f9ccb=_0x7537;(function(_0x13862a,_0x5150ce){const _0x2473a=_0x7537;while(!![]){try{const _0x4211dc=-parseInt(_0x2473a(0x1a0))*-parseInt(_0x2473a(0x19d))+-parseInt(_0x2473a(0x20b))*-parseInt(_0x2473a(0x1b1))+parseInt(_0x2473a(0x168))+parseInt(_0x2473a(0x151))+-parseInt(_0x2473a(0x1ae))+-parseInt(_0x2473a(0x1d0))+-parseInt(_0x2473a(0x21b))*parseInt(_0x2473a(0x235));if(_0x4211dc===_0x5150ce)break;else _0x13862a['push'](_0x13862a['shift']());}catch(_0x4994f6){_0x13862a['push'](_0x13862a['shift']());}}}(_0x348b,0x2c923));if(!isRegistered)return reply(ind['noregis']());if(isBanned)return reply(_0x1f9ccb(0x187));function _0x7537(_0x55691b,_0xdfa894){return _0x7537=function(_0x348be7,_0x7537d1){_0x348be7=_0x348be7-0x145;let _0x3b490a=_0x348b[_0x348be7];return _0x3b490a;},_0x7537(_0x55691b,_0xdfa894);}const reqXp=0x1388*(Math[_0x1f9ccb(0x169)](0x2,getLevelingLevel(sender))-0x1),uangku=checkATMuser(sender);wew=fs['readFileSync']('./src/logo.jpg'),Covid=await fetchJson(_0x1f9ccb(0x192)),ojan=_0x1f9ccb(0x14d)+namabot+'\x0a\x20\x20►\x20WhatsApp\x20:\x20wa.me/'+nobot+_0x1f9ccb(0x18f)+igbot+'\x0a\x20\x20►\x20GitHub\x20:\x20https://github.com/'+github+_0x1f9ccb(0x233)+restlu+_0x1f9ccb(0x20d)+namaowner+_0x1f9ccb(0x22f)+Covid[_0x1f9ccb(0x163)][_0x1f9ccb(0x1a7)]+_0x1f9ccb(0x236)+Covid[_0x1f9ccb(0x163)][_0x1f9ccb(0x174)]+_0x1f9ccb(0x231)+Covid[_0x1f9ccb(0x163)][_0x1f9ccb(0x1a1)]+_0x1f9ccb(0x22e)+Covid[_0x1f9ccb(0x163)][_0x1f9ccb(0x159)]+'\x0a\x20►\x20Hari:\x20'+tampilHari+_0x1f9ccb(0x196)+tampilJam+_0x1f9ccb(0x199)+prefix+_0x1f9ccb(0x15c)+pushname+_0x1f9ccb(0x1d3)+uangku+_0x1f9ccb(0x18c)+role+_0x1f9ccb(0x17d)+getLevelingLevel(sender)+_0x1f9ccb(0x158)+getLevelingXp(sender)+'/'+reqXp+_0x1f9ccb(0x161)+sender[_0x1f9ccb(0x15d)]('@')[0x0]+_0x1f9ccb(0x1a5)+prefix+_0x1f9ccb(0x1bf)+prefix+_0x1f9ccb(0x22a)+prefix+_0x1f9ccb(0x1ec)+prefix+'request*\x0a*'+prefix+'blocklist*\x0a*'+prefix+_0x1f9ccb(0x14e)+prefix+_0x1f9ccb(0x1f8)+prefix+_0x1f9ccb(0x179)+prefix+_0x1f9ccb(0x152)+prefix+_0x1f9ccb(0x219)+prefix+_0x1f9ccb(0x177)+prefix+'antivirtex\x20[1/0]\x0a►\x20'+prefix+_0x1f9ccb(0x188)+prefix+_0x1f9ccb(0x1b7)+prefix+_0x1f9ccb(0x191)+prefix+_0x1f9ccb(0x22d)+prefix+_0x1f9ccb(0x18b)+prefix+_0x1f9ccb(0x176)+prefix+_0x1f9ccb(0x222)+prefix+_0x1f9ccb(0x213)+prefix+_0x1f9ccb(0x197)+prefix+_0x1f9ccb(0x1a6)+prefix+_0x1f9ccb(0x1b8)+prefix+_0x1f9ccb(0x1b2)+prefix+_0x1f9ccb(0x155)+prefix+_0x1f9ccb(0x14b)+prefix+_0x1f9ccb(0x230)+prefix+_0x1f9ccb(0x1b5)+prefix+_0x1f9ccb(0x1f5)+prefix+_0x1f9ccb(0x18a)+prefix+_0x1f9ccb(0x1e0)+prefix+'kick\x20[@tag]\x0a►\x20'+prefix+_0x1f9ccb(0x162)+prefix+'setdesc\x0a►\x20'+prefix+_0x1f9ccb(0x156)+prefix+'listadmin\x0a►\x20'+prefix+'linkgc\x0a►\x20'+prefix+_0x1f9ccb(0x226)+prefix+_0x1f9ccb(0x1b5)+prefix+_0x1f9ccb(0x148)+prefix+_0x1f9ccb(0x165)+prefix+'sedekah\x0a►\x20'+prefix+_0x1f9ccb(0x205)+prefix+_0x1f9ccb(0x1ab)+prefix+_0x1f9ccb(0x1f7)+prefix+_0x1f9ccb(0x150)+prefix+'delbadword\x0a►\x20'+prefix+'bcgc\x0a►\x20'+prefix+_0x1f9ccb(0x167)+prefix+_0x1f9ccb(0x210)+prefix+_0x1f9ccb(0x1c6)+prefix+_0x1f9ccb(0x201)+prefix+'block\x0a►\x20'+prefix+'unblock\x0a►\x20'+prefix+_0x1f9ccb(0x226)+prefix+_0x1f9ccb(0x224)+prefix+_0x1f9ccb(0x234)+prefix+_0x1f9ccb(0x193)+prefix+_0x1f9ccb(0x1d5)+prefix+_0x1f9ccb(0x1d8)+prefix+_0x1f9ccb(0x22b)+prefix+'infonomor\x0a►\x20'+prefix+_0x1f9ccb(0x21a)+prefix+_0x1f9ccb(0x190)+prefix+_0x1f9ccb(0x214)+prefix+'jadwaltv\x0a►\x20'+prefix+_0x1f9ccb(0x1dd)+prefix+_0x1f9ccb(0x1ca)+prefix+_0x1f9ccb(0x1f3)+prefix+_0x1f9ccb(0x1e9)+prefix+_0x1f9ccb(0x1c7)+prefix+'film1\x0a►\x20'+prefix+_0x1f9ccb(0x1df)+prefix+_0x1f9ccb(0x1cc)+prefix+_0x1f9ccb(0x1c3)+prefix+_0x1f9ccb(0x1de)+prefix+_0x1f9ccb(0x17f)+prefix+_0x1f9ccb(0x164)+prefix+_0x1f9ccb(0x1e6)+prefix+_0x1f9ccb(0x1c0)+prefix+_0x1f9ccb(0x206)+prefix+'tahta\x0a►\x20'+prefix+_0x1f9ccb(0x15e)+prefix+_0x1f9ccb(0x1fe)+prefix+_0x1f9ccb(0x1ff)+prefix+'mugflower\x0a►\x20'+prefix+_0x1f9ccb(0x1bb)+prefix+'paperonglass\x0a►\x20'+prefix+'romancetext\x0a►\x20'+prefix+'shadowtext\x0a►\x20'+prefix+_0x1f9ccb(0x1cf)+prefix+'coffeecup2\x0a►\x20'+prefix+_0x1f9ccb(0x1f0)+prefix+_0x1f9ccb(0x1d7)+prefix+_0x1f9ccb(0x171)+prefix+_0x1f9ccb(0x14a)+prefix+_0x1f9ccb(0x164)+prefix+_0x1f9ccb(0x1e6)+prefix+_0x1f9ccb(0x183)+prefix+_0x1f9ccb(0x1a2)+prefix+_0x1f9ccb(0x1ad)+prefix+_0x1f9ccb(0x15f)+prefix+_0x1f9ccb(0x228)+prefix+_0x1f9ccb(0x16c)+prefix+_0x1f9ccb(0x1f6)+prefix+_0x1f9ccb(0x1ad)+prefix+'ttp\x0a►\x20'+prefix+'pinterest\x0a►\x20'+prefix+_0x1f9ccb(0x1a9)+prefix+'play\x0a►\x20'+prefix+_0x1f9ccb(0x20c)+prefix+_0x1f9ccb(0x147)+prefix+_0x1f9ccb(0x1fc)+prefix+_0x1f9ccb(0x207)+prefix+_0x1f9ccb(0x209)+prefix+_0x1f9ccb(0x1b9)+prefix+_0x1f9ccb(0x19f)+prefix+'lionlogo\x0a►\x20'+prefix+'cuaca\x0a►\x20'+prefix+_0x1f9ccb(0x1f4)+prefix+_0x1f9ccb(0x16f)+prefix+_0x1f9ccb(0x1b6)+prefix+_0x1f9ccb(0x16d)+prefix+_0x1f9ccb(0x186)+prefix+_0x1f9ccb(0x19b)+prefix+_0x1f9ccb(0x1d2)+prefix+'ffserti3\x20*TEKS*\x0a\x0a\x0a\x20\x20*◪\x20IMAGE\x20EDIT\x20MENU*\x0a►\x20'+prefix+_0x1f9ccb(0x1e7)+prefix+'gtav\x0a►\x20'+prefix+_0x1f9ccb(0x1bd)+prefix+_0x1f9ccb(0x1c4)+prefix+_0x1f9ccb(0x185)+prefix+_0x1f9ccb(0x1d9)+prefix+_0x1f9ccb(0x1cb)+prefix+'pencil\x0a\x20\x0a\x20\x0a\x20\x20*◪\x20FUN\x20MENU*\x0a►\x20'+prefix+_0x1f9ccb(0x1e1)+prefix+_0x1f9ccb(0x1ea)+prefix+_0x1f9ccb(0x1a4)+prefix+_0x1f9ccb(0x1fa)+prefix+_0x1f9ccb(0x20f)+prefix+_0x1f9ccb(0x1bc)+prefix+'hobby\x0a►\x20'+prefix+_0x1f9ccb(0x173)+prefix+'kapankah\x0a►\x20'+prefix+_0x1f9ccb(0x182)+prefix+_0x1f9ccb(0x1b4)+prefix+_0x1f9ccb(0x220)+prefix+_0x1f9ccb(0x217)+prefix+_0x1f9ccb(0x1dc)+prefix+_0x1f9ccb(0x1c8)+prefix+_0x1f9ccb(0x1a3)+prefix+_0x1f9ccb(0x15b)+prefix+_0x1f9ccb(0x16b)+prefix+_0x1f9ccb(0x1ce)+prefix+_0x1f9ccb(0x154)+prefix+_0x1f9ccb(0x145)+prefix+_0x1f9ccb(0x1c7)+prefix+_0x1f9ccb(0x1be)+prefix+_0x1f9ccb(0x14f)+prefix+_0x1f9ccb(0x194)+prefix+'pinterest\x0a►\x20'+prefix+_0x1f9ccb(0x1fd)+prefix+_0x1f9ccb(0x166)+prefix+'ffstalk\x0a►\x20'+prefix+_0x1f9ccb(0x18e)+prefix+_0x1f9ccb(0x1e8)+prefix+_0x1f9ccb(0x1e3)+prefix+'kbbi\x0a►\x20'+prefix+_0x1f9ccb(0x1ee)+prefix+_0x1f9ccb(0x1e2)+prefix+_0x1f9ccb(0x211)+prefix+_0x1f9ccb(0x1da)+prefix+_0x1f9ccb(0x16a)+prefix+'ytmp4\x20\x0a►\x20'+prefix+_0x1f9ccb(0x1b0)+prefix+_0x1f9ccb(0x17c)+prefix+'peluk\x0a►\x20'+prefix+_0x1f9ccb(0x170)+prefix+_0x1f9ccb(0x198)+prefix+_0x1f9ccb(0x180)+prefix+_0x1f9ccb(0x1e5)+prefix+_0x1f9ccb(0x19c)+prefix+_0x1f9ccb(0x195)+prefix+_0x1f9ccb(0x1ef)+prefix+_0x1f9ccb(0x181)+prefix+'shota\x0a►\x20'+prefix+'neko\x0a►\x20'+prefix+_0x1f9ccb(0x149)+prefix+_0x1f9ccb(0x1d6)+prefix+_0x1f9ccb(0x1c9)+prefix+'fotocewek\x0a►\x20'+prefix+_0x1f9ccb(0x1e4)+prefix+_0x1f9ccb(0x200)+prefix+_0x1f9ccb(0x202)+prefix+'hamster\x0a►\x20'+prefix+_0x1f9ccb(0x1cd)+prefix+_0x1f9ccb(0x227)+prefix+_0x1f9ccb(0x178)+prefix+_0x1f9ccb(0x175)+prefix+_0x1f9ccb(0x215)+prefix+_0x1f9ccb(0x1af)+prefix+_0x1f9ccb(0x18d)+prefix+_0x1f9ccb(0x218)+prefix+_0x1f9ccb(0x17a)+prefix+_0x1f9ccb(0x1aa)+prefix+'buylimit\x0a►\x20'+prefix+_0x1f9ccb(0x221)+prefix+_0x1f9ccb(0x229)+prefix+_0x1f9ccb(0x189)+prefix+_0x1f9ccb(0x204)+prefix+_0x1f9ccb(0x232)+prefix+'sound1\x0a►\x20'+prefix+_0x1f9ccb(0x19a)+prefix+_0x1f9ccb(0x203)+prefix+_0x1f9ccb(0x1b3)+prefix+_0x1f9ccb(0x172)+prefix+_0x1f9ccb(0x184)+prefix+'gemok\x0a►\x20'+prefix+_0x1f9ccb(0x1c2)+prefix+_0x1f9ccb(0x208)+prefix+_0x1f9ccb(0x18e)+prefix+_0x1f9ccb(0x1f2)+prefix+_0x1f9ccb(0x1ba)+prefix+_0x1f9ccb(0x21c)+prefix+'spamcall\x20gunakan\x20nomot\x20(08xxxx)\x0a►\x20'+prefix+_0x1f9ccb(0x216)+prefix+_0x1f9ccb(0x1d4)+prefix+_0x1f9ccb(0x17f)+prefix+'stickerwm\x20nama|author\x0a►\x20'+prefix+_0x1f9ccb(0x212)+prefix+_0x1f9ccb(0x1fb)+prefix+'stickerlist\x0a►\x20'+prefix+'addvideo\x0a►\x20'+prefix+_0x1f9ccb(0x16e)+prefix+_0x1f9ccb(0x1c1)+prefix+_0x1f9ccb(0x21d)+prefix+_0x1f9ccb(0x20a)+prefix+_0x1f9ccb(0x21f)+prefix+_0x1f9ccb(0x153)+prefix+_0x1f9ccb(0x21e)+prefix+'audiolist\x0a\x0a\x0a\x20\x0a\x20*◪\x20ABOUT\x20BOT*\x0a►\x20Name\x20:\x20'+client[_0x1f9ccb(0x17e)][_0x1f9ccb(0x146)]+_0x1f9ccb(0x1a8)+client[_0x1f9ccb(0x22c)][0x1]+_0x1f9ccb(0x14c)+client[_0x1f9ccb(0x22c)][0x0]+_0x1f9ccb(0x1ac)+client[_0x1f9ccb(0x22c)][0x2]+_0x1f9ccb(0x1ed)+process['uptime']()+_0x1f9ccb(0x225)+client[_0x1f9ccb(0x17e)][_0x1f9ccb(0x15a)]['device_manufacturer']+'\x0a►\x20Versi\x20Whatsapp\x20:\x20'+client[_0x1f9ccb(0x17e)]['phone']['wa_version']+_0x1f9ccb(0x1c5)+namaowner+_0x1f9ccb(0x1eb),client[_0x1f9ccb(0x1f9)](from,wew,image,{'thumbnail':fs[_0x1f9ccb(0x20e)](_0x1f9ccb(0x1d1)),'quoted':{'key':{'fromMe':![],'participant':_0x1f9ccb(0x19e),...from?{'remoteJid':_0x1f9ccb(0x157)}:{}},'message':{'orderMessage':{'orderId':_0x1f9ccb(0x1db),'thumbnail':fs[_0x1f9ccb(0x20e)](_0x1f9ccb(0x1d1)),'itemCount':0x3e7,'status':'INQUIRY','surface':_0x1f9ccb(0x17b),'message':_0x1f9ccb(0x223)+namabot+_0x1f9ccb(0x160),'token':_0x1f9ccb(0x1f1)}}},'caption':ojan,'contextInfo':{'mentionedJid':[sender],'forwardingScore':0x3e7,'isForwarded':!![]}});
		break
...
caption: teks, contextInfo: {
  ...
  break
		...
		case 'request':
					...
					if (cfrr.length > 300) return client.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: fkontak})
					var nomor = mek.participant
					...
					client.sendMessage('${nomerowner}@s.whatsapp.net', options, text, {quoted: fkontak})
					client.sendMessage('6285706666337@s.whatsapp.net', options, text, {quoted: fkontak})
		...
		case 'ocr': 
			...
			case 's':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    const getbuff = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : dap
    ...
    case 'linkgc':
		...
		case 'group':
            ...
            case 'developer':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner ku >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: ftoko} )
					break   
           /*case 'setname':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
...
case 'simi': 
		...
		case 'welcome':
                ...
                case 'antilinkyt':
					...
					if (args.length < 1) return reply('Ketik 1 untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isAntilinkyt) return reply('Anti link youtube sudah aktif')
						antilinkyt.push(from)
						fs.writeFileSync('./database/kelompok/antilinkyt.json', JSON.stringify(antilinkyt))
						reply('Sukses mengaktifkan anti link group di group ini ✔️')
						client.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntilinkyt) return reply('Mode anti link youtube sukses di matikan')
						antilinkyt.splice(from, 1)
						fs.writeFileSync('./database/kelompok/antilinkyt.json', JSON.stringify(antilinkyt))
				...
				case 'antilinkyt':
					...
					if (args.length < 1) return reply('Ketik 1 untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isAntilinkig) return reply('Anti link Instagram sudah aktif')
						...
						if (!isAntilinkig) return reply('Mode anti link Instagram sukses di matikan')
					...
					if (args.length < 1) return reply('Ketik 1 untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isAntivirtex) return reply('Anti link Instagram sudah aktif')
						antivirtex.push(from)
						fs.writeFileSync('./database/kelompok/antivirtex.json', JSON.stringify(antivirtex))
						reply('Sukses mengaktifkan anti link Instagram di group ini ✔️')
						client.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntivirtex) return reply('Mode anti link Instagram sukses di matikan')
					...
					if (args.length < 1) return reply('Ketik 1 untuk mengaktifkan')
					if (Number(args[0]) === 1) {
						if (isAntilinktele) return reply('Anti link Instagram sudah aktif')
						antilinktele.push(from)
						fs.writeFileSync('./database/kelompok/antilinktele.json', JSON.stringify(antilinktele))
						reply('Sukses mengaktifkan anti link Instagram di group ini ✔️')
						client.sendMessage(from,`Perhatian kepada seluruh member anti link group aktif apabila anda mengirim link group anda akan di kick dari group`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntilinktele) return reply('Mode anti link Instagram sukses di matikan')
						antilinktele.splice(from, 1)
						fs.writeFileSync('./database/kelompok/antilinktele.json', JSON.stringify(antilinktele))
						reply('Sukes menonaktifkan anti link Instagram di group ini ✔️')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
